...
function formatPokerHand(hand, held = new Set()) {
  return hand
    .map((card, index) => `${formatPokerCard(card)}${held.has(index) ? ' ✅' : ''}`)
    .join('  ');
}
...
client.on('interactionCreate', async interaction => {
  ...
  const originalReply = originalInteraction.reply.bind(originalInteraction);
  const originalDeferReply = originalInteraction.deferReply.bind(originalInteraction);
  let deferredReply;
  const acknowledgeInteraction = () => {
    if (originalInteraction.deferred || originalInteraction.replied) {
      return Promise.resolve();
    }
    deferredReply ||= originalDeferReply();
    return deferredReply;
  };
  const acknowledgementTimeout = setTimeout(() => {
    acknowledgeInteraction().catch((err) => {
      console.error('Interaction acknowledgement error:', err);
    });
  }, 1500);
    ...
    get(target, property) {
      if (property === 'reply') {
        return (response) => {
          ...
          if (deferredReply || target.deferred) {
            return acknowledgeInteraction().then(() => target.editReply(response));
          }
          return originalReply(response);
        };
      }
